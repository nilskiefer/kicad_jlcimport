...
class TestWriteFootprint:
    ...
    def test_smd_attribute(self):
        pad = EEPad(shape="RECT", x=0, y=0, width=1, height=1,
                    layer="1", number="1", drill=0)
        fp = _make_footprint(pads=[pad])
        result = write_footprint(fp, "Test")
        assert "(attr smd)" in result

    def test_tht_attribute(self):
        pad = EEPad(shape="OVAL", x=0, y=0, width=1, height=1,
                    layer="11", number="1", drill=0.8)
        fp = _make_footprint(pads=[pad])
        result = write_footprint(fp, "Test")
        assert "(attr through_hole)" in result

    def test_pad_generation(self):
        pad = EEPad(shape="RECT", x=1.0, y=2.0, width=0.5, height=0.8,
                    layer="1", number="1", drill=0)
        fp = _make_footprint(pads=[pad])
        result = write_footprint(fp, "Test")
    ...
    def test_pad_with_drill(self):
        pad = EEPad(shape="OVAL", x=0, y=0, width=1.5, height=1.5,
                    layer="11", number="A1", drill=1.0)
        fp = _make_footprint(pads=[pad])
        result = write_footprint(fp, "Test")
    ...
    def test_pad_with_rotation(self):
        pad = EEPad(shape="RECT", x=0, y=0, width=1, height=2,
                    layer="1", number="1", drill=0, rotation=45.0)
        fp = _make_footprint(pads=[pad])
        result = write_footprint(fp, "Test")
        assert "45" in result

    def test_track_generation(self):
        track = EETrack(width=0.2, layer="F.SilkS",
                        points=[(0, 0), (1, 0), (1, 1)])
    ...
    def test_properties_added(self):
        fp = _make_footprint()
        result = write_footprint(fp, "Test", lcsc_id="C123456",
    ...
    def test_uuid_uniqueness(self):
        pad1 = EEPad(shape="RECT", x=0, y=0, width=1, height=1,
                     layer="1", number="1", drill=0)
        pad2 = EEPad(shape="RECT", x=2, y=0, width=1, height=1,
                     layer="1", number="2", drill=0)
